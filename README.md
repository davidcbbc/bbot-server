...
Add the flag on the **server side** in your BBOT Server config file (the same place you configure API keys), for example:
...
For Docker deployments, add the same stanza to the mounted config file or provide it via `BBOT_CONFIG_PATH` so the server picks it up at startup.
